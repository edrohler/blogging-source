...
import { FilterDescriptor } from '@src/app/core/data/state/filter-descriptor.interface';
...
export class EntityDataService<T> extends BaseDataService<T, KinveyServiceConfig, EntityState> {
        ...
        return this.coreService.save(this.config, item)
            .then((newItem: any) => {
                return this.toPromise(() => {
                    ...
                    let state = this.dataState.getCurrent() as EntityState;
    ...
    protected getDataChanges(): Observable<T> {
        const entityObservable = state => {
            ...
            if (state.id) {
                obs = this.coreService.findById(this.config, state.id).pipe(
                    this.handleObservableError(),
                    last(value => !!value),
                    map(item => this.mapData(item))
                );
            } else {
                obs = this.coreService.find(this.config, { filter: state.filter, take: 1 }).pipe(
                    this.handleObservableError(),
                    last(value => !!value),
